...
public class EmployeeView {
	...
	public void selectAll() {
		...
		//DB 에서 전체 사원 정보를 조회하여 List<Employee> 형태로 반환하는
	...
	public void updateEmployee() {
		...
		// 기본생성자로 객체 생성후 setter를 이용하여 초기화
		...
		if(result > 0) {
			System.out.println("사원 정보가 수정 되었습니다.");
		} else {
	...
	public void selectDeptTotalSalary() {
		...
		for(String key : dao.selectDeptTotalSalary().keySet()) {
			int salaryInteger = dao.selectDeptTotalSalary().get(key);
			System.out.printf("부서 : %s    |    급여 : %d\n", key, salaryInteger);
		}
	...
	public void selectJobAvgSalary() {
		...
		for(String key : dao.selectJobAvgSalary().keySet()) {
			int salary = dao.selectJobAvgSalary().get(key);
			System.out.printf("직급 : %s   |   평균급여 : %d \n",key, salary);
		}
